package com.qinyadan.system.dsp.protocol.utils;

import io.netty.buffer.ByteBuf;
import java.nio.charset.StandardCharsets;
import java.util.Objects;

import static com.qinyadan.system.dsp.constant.ColumnTypeConstants.*;


public class IOUtils {

    public static final byte END_FLAG = 0x00;

    public static void writeString(String s, ByteBuf byteBuf) {
        if (Objects.isNull(s)) {
            return;
        }

        byte[] bytes = s.getBytes(StandardCharsets.UTF_8);

        byteBuf.writeBytes(bytes);
        byteBuf.writeByte(END_FLAG);
    }

    public static void writeInteger(int s, ByteBuf byteBuf, int length) {
        if (length < 1 || length > 4) {
            throw new IllegalArgumentException("Unsupported integer width: " + length);
        }
        long maximum = (1L << (length * 8)) - 1;
        if (s < 0 || s > maximum) {
            throw new IllegalArgumentException(
                    "Unsigned integer does not fit in " + length + " bytes: " + s);
        }

        byte[] bytes = new byte[length];
        for (int i = length - 1; i >= 0; i--) {
            bytes[i] = (byte) ((s >> (i * 8)) & 0xff);
        }

        byteBuf.writeBytes(bytes);
    }

    public static void writeInteger4(int s, ByteBuf byteBuf) {
        //最高位
        byte b1 = (byte) ((s >> 24) & 0xff);
        //次高位
        byte b2 = (byte) ((s >> 16) & 0xff);
        byte b3 = (byte) ((s >> 8) & 0xff);
        byte b4 = (byte) (s & 0xff);

        byteBuf.writeByte(b4);
        byteBuf.writeByte(b3);
        byteBuf.writeByte(b2);
        byteBuf.writeByte(b1);
    }

    public static void writeInteger3(int s, ByteBuf byteBuf) {
        if (s < 0 || s > 0xffffff) {
            throw new IllegalArgumentException("Unsigned integer does not fit in 3 bytes: " + s);
        }

        //次高位
        byte b2 = (byte) ((s >> 16) & 0xff);
        byte b3 = (byte) ((s >> 8) & 0xff);
        byte b4 = (byte) (s & 0xff);

        byteBuf.writeByte(b4);
        byteBuf.writeByte(b3);
        byteBuf.writeByte(b2);
    }


    public static void writeInteger2(int s, ByteBuf byteBuf) {
        if (s < 0 || s > 0xffff) {
            throw new IllegalArgumentException("Unsigned integer does not fit in 2 bytes: " + s);
        }

        byte b3 = (byte) ((s >> 8) & 0xff);
        byte b4 = (byte) (s & 0xff);

        byteBuf.writeByte(b4);
        byteBuf.writeByte(b3);
    }

    public static void writeLengthEncodedInteger(int s, ByteBuf byteBuf) {
        if (s < 0) {
            throw new IllegalArgumentException("Length-encoded integer cannot be negative: " + s);
        }
        if (s < 251) {
            IOUtils.writeInteger(s, byteBuf, 1);
        } else if (s <= 0xffff) {
            IOUtils.writeByte((byte) MYSQL_TYPE_BLOB, byteBuf);
            IOUtils.writeInteger(s, byteBuf, 2);
        } else if (s <= 0xffffff) {
            IOUtils.writeByte((byte) MYSQL_TYPE_VAR_STRING, byteBuf);
            IOUtils.writeInteger(s, byteBuf, 3);
        } else {
            IOUtils.writeByte((byte) MYSQL_TYPE_STRING, byteBuf);
            byteBuf.writeLongLE(s & 0xffffffffL);
        }
    }

    public static void writeShort(short s, ByteBuf byteBuf) {
        byte b3 = (byte) ((s >> 8) & 0xff);
        byte b4 = (byte) (s & 0xff);

        byteBuf.writeByte(b4);
        byteBuf.writeByte(b3);
    }

    public static void writeBytes(byte[] s, ByteBuf byteBuf) {
        byteBuf.writeBytes(s);
        byteBuf.writeByte(END_FLAG);
    }

    public static void writeBytesWithoutEndFlag(byte[] s, ByteBuf byteBuf) {
        byteBuf.writeBytes(s);
    }

    public static void writeByte(byte s, ByteBuf byteBuf) {
        byteBuf.writeByte(s);
    }


    public static void writeLengthEncodedString(ByteBuf byteBuf, String string) {
        if (Objects.isNull(string)) {
            byteBuf.writeByte(0xfb);
            return;
        }

        byte[] bytes = string.getBytes(StandardCharsets.UTF_8);
        IOUtils.writeLengthEncodedInteger(bytes.length, byteBuf);
        if (bytes.length > 0) {
            IOUtils.writeBytesWithoutEndFlag(bytes, byteBuf);
        }
    }


    //read section
    public static short readShort(ByteBuf byteBuf) {
        byte low = (byte) (byteBuf.readByte() & 0xff);
        byte high = (byte) (byteBuf.readByte() & 0xff);

        return (short) (((high & 0xff) << 8) | (low & 0xff));
    }

    /**
     * @param byteBuf
     * @return
     */
    public static String readString(ByteBuf byteBuf) {
        int length = byteBuf.bytesBefore(END_FLAG);
        if (length < 0) {
            length = byteBuf.readableBytes();
        }
        byte[] byteResult = new byte[length];
        byteBuf.readBytes(byteResult);
        if (byteBuf.isReadable() && byteBuf.getByte(byteBuf.readerIndex()) == END_FLAG) {
            byteBuf.skipBytes(1);
        }
        return new String(byteResult, StandardCharsets.UTF_8);
    }

    /**
     * See https://dev.mysql.com/doc/internals/en/string.html#packet-Protocol::RestOfPacketString
     *
     * @param byteBuf
     * @return
     */
    public static String readEofString(ByteBuf byteBuf) {
        int length = byteBuf.readableBytes();
        byte[] byteResult = new byte[length];
        byteBuf.readBytes(byteResult);
        return new String(byteResult, StandardCharsets.UTF_8);
    }

    /**
     * length start from 4 to 1
     *
     * @param byteBuf
     * @param length
     * @return
     */
    public static int readInteger(ByteBuf byteBuf, int length) {
        byte[] resultArray = new byte[length];
        for (int i = 0; i < length; i++) {
            resultArray[i] = byteBuf.readByte();
        }

        int result = 0;
        for (int i = 0; i < length; i++) {
            result += (resultArray[i] & 0xff) << (8 * i);
        }

        return result;
    }

    public static byte readByte(ByteBuf byteBuf) {
        return byteBuf.readByte();
    }

    public static int readLengthEncodedInteger(ByteBuf byteBuf) {

        int firstByteInt = IOUtils.readInteger(byteBuf, 1);
        if (firstByteInt < 251) {
            return firstByteInt;
        } else if (firstByteInt == 0xfb) {
            return -1;
        } else if (firstByteInt == MYSQL_TYPE_BLOB) {
            return IOUtils.readInteger(byteBuf, 2);
        } else if (firstByteInt == MYSQL_TYPE_VAR_STRING) {
            return IOUtils.readInteger(byteBuf, 3);
        } else if (firstByteInt == MYSQL_TYPE_STRING) {
            long value = byteBuf.readLongLE();
            if (value > Integer.MAX_VALUE) {
                throw new IllegalArgumentException("Length exceeds supported packet size: " + value);
            }
            return (int) value;
        }
        throw new IllegalArgumentException("Invalid length-encoded integer prefix: " + firstByteInt);
    }


    public static byte[] readBytes(ByteBuf byteBuf, int s) {
        byte[] result = new byte[s];

        for (int i = 0; i < s; i++) {
            result[i] = byteBuf.readByte();
        }

        return result;
    }

    public static byte[] readLengthEncodedBytes(ByteBuf byteBuf) {
        int length = readLengthEncodedInteger(byteBuf);
        if (length < 0) {
            return null;
        }
        return readBytes(byteBuf, length);
    }

    public static byte[] readBytesUntilNull(ByteBuf byteBuf) {
        int length = byteBuf.bytesBefore(END_FLAG);
        if (length < 0) {
            length = byteBuf.readableBytes();
        }
        byte[] result = readBytes(byteBuf, length);
        if (byteBuf.isReadable() && byteBuf.getByte(byteBuf.readerIndex()) == END_FLAG) {
            byteBuf.skipBytes(1);
        }
        return result;
    }

    public static String readLengthEncodedString(ByteBuf byteBuf) {
        int length = readLengthEncodedInteger(byteBuf);
        if (length < 0) {
            return null;
        }
        return new String(readBytes(byteBuf, length), StandardCharsets.UTF_8);
    }

    public static String readFixLengthString(ByteBuf buf, int length) {
        byte[] bytes = new byte[length];
        buf.readBytes(bytes);
        return new String(bytes, StandardCharsets.UTF_8);
    }

    public static ByteBuf copyByteBuf(ByteBuf buf) {
        ByteBuf byteBuf = buf.alloc().buffer(128);

        int i = 0;
        int length = buf.readableBytes();
        while (i++ < length) {
            byteBuf.writeByte(buf.readByte());
        }

        return byteBuf;
    }

    /**
     * 小端
     */
    public static byte[] getBytes(int v) {
        byte[] result = new byte[4];
        for (int i = 0; i < 4; i++) {
            result[i] = (byte) (v & 0xff);
            v = v >> 8;
        }

        return result;
    }

}
